...
class DurationPickerResult {
  ...
  const DurationPickerResult({required this.durationSeconds, this.sessionName});
}
...
class DurationPickerSheet extends StatefulWidget {
  ...
  final bool showSessionNameInput;
  final ValueChanged<DurationPickerResult> onSelected;
...
class _DurationPickerSheetState extends State<DurationPickerSheet> {
  ...
  String? get _sanitizedSessionName {
    if (!widget.showSessionNameInput) {
      return null;
    }

    final trimmed = _sessionNameController.text.trim();
    return trimmed.isEmpty ? null : trimmed;
  }

  void _submitSelection(int durationSeconds) {
    widget.onSelected(
      DurationPickerResult(
        durationSeconds: durationSeconds,
        sessionName: _sanitizedSessionName,
      ),
    );
  }
              ...
              if (widget.showSessionNameInput) ...[
                TextField(
                  ...
                  decoration: InputDecoration(
                    labelText: l10n.sessionNameOptional,
                ...
                Text(l10n.sessionNameHelper, style: typography.bodySmall),
                SizedBox(height: dimens.spacingXL),
              ],
              if (_isCustomMode) ...[
                Container(
                  ...
                  children: widget.options
                      .map((duration) {
                        ...
                        return GestureDetector(
                          onTap: () => _submitSelection(duration),
                          child: Container(
                            padding: EdgeInsets.symmetric(
